...
$Script = "Set-DesktopBackground.ps1"

# If local path for script doesn't exist, create it
If (!(Test-Path $Target)) { New-Item -Path $Target -Type Directory -Force }

Copy-Item "Set-DesktopBackground.ps1" -Destination "C:\ProgramData\Scripts" -Force
...
reg.exe add "HKU\DEFAULT_USER\Software\Microsoft\Windows\CurrentVersion\RunOnce" /v "DefaultBackground" /t REG_SZ /d "powershell.exe -executionpolicy Bypass -Windowstyle Hidden -file C:\Programdata\Scripts\Set-DesktopBackground.ps1" /f | Out-Host
